<#
.SYNOPSIS
    Scrape AirBnB accommodations for a given location and date range.

.DESCRIPTION
    Activates the project virtual environment (if present), then invokes the
    Bookinz AirBnB pipeline to scrape accommodations from AirBnB and store the
    results in the AirBnB bronze layer (Parquet files on disk, queryable via DuckDB).

.PARAMETER Location
    City or region to search (e.g. "Tirana, Albania", "Rome, Italy").
...
.EXAMPLE
    .\run_airbnb.ps1 -Location "Tirana, Albania" -CheckIn 2026-09-20 -CheckOut 2026-09-27

.EXAMPLE
    .\run_airbnb.ps1 -Location "Rome, Italy" -CheckIn 2026-09-20 -CheckOut 2026-09-27 -NumAdults 4 -MaxPages 3 -Delay 2.5
#>

[CmdletBinding()]
param(
    [Parameter(Mandatory = $true, HelpMessage = "City or region to search (e.g. 'Tirana, Albania').")]
...
function Write-Info  ([string]$Msg) { Write-Host "[airbnb] $Msg" -ForegroundColor Cyan }
function Write-Warn  ([string]$Msg) { Write-Host "[airbnb] WARNING: $Msg" -ForegroundColor Yellow }
function Write-Fail  ([string]$Msg) { Write-Host "[airbnb] ERROR: $Msg" -ForegroundColor Red }
...
$PipelineArgs = @(
    "-m", "bookinz.pipeline.airbnb_pipeline",
    "--area",       $Location,
    "--checkin",    $CheckIn,
    "--checkout",   $CheckOut,
    "--data-path",  $DataPath,
